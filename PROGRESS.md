...
- ✅ Slice 3 — Frontend: adaptive re-split-on-timeout retry
  - Backstop for a page so dense it still exceeds the 45s cap even on Haiku. `PhotoImport.jsx` `importBatch` helper: when a PDF batch fails with a **non-terminal** error (anything other than `parse_error`/`claude_error`/`image_too_large`/`forbidden` — e.g. a 504 gateway timeout), it re-splits **that** batch into single pages (`splitPdf(b64, {pagesPerBatch:1})`) and retries each, merging the candidates. Bottoms out at 1 page (a `[self]` re-split → surfaces the mapped error), so no retry storm; terminal errors and non-PDF imports surface immediately
  - Tests: 2 in `PhotoImport.test.jsx` (AC13 re-split+retry+merge, AC14 single-page failure surfaces with no infinite retry); existing AC11 confirms terminal `claude_error` is **not** retried. 636 frontend pass. Coverage `PhotoImport.jsx` 97.52 lines / 100 funcs (Tier B). No new deps, no i18n (re-uses existing error mapping). Frontend-only — auth/Claude/secret seams untouched
  - Deferred (out of scope): bring-your-own Azure Functions (permanent fix to remove the 45s cap; would let chunking be dropped and Sonnet restored for best quality); same mitigation for dense single-image / large-pptx imports; parallel batch dispatch

### Post-v1 — Modern Greek (`el`)
- ✅ Added Modern Greek as a first-class language (frontend-only). `el-GR` in `CourseList` `LANGUAGES`; `el` ("Greek"/"Grieks") in `CourseList` `SIDE_LANGS` and `PhotoImport` `LANG_OPTIONS`; `courses.sideLang.el` in en + nl locales. Closes the gap where a Modern-Greek textbook could only be tagged `grc` (Ancient Greek). 🔊 TTS works (browsers ship a Greek voice; `tts.js` prefix-matches `el`). Backend BCP-47 validator already accepted `el` — no API change
  - Tests: 4 (3 in `CourseList.test.jsx`, 1 in `PhotoImport.test.jsx`). 640 frontend pass. Coverage `CourseList.jsx` 98.74 / `PhotoImport.jsx` 97.53 lines / `strings.js` 100. No new deps. Security scan N/A (no auth/Claude/secret seam touched). Plan: [docs/plans/done/post-v1-modern-greek-language.md](docs/plans/done/post-v1-modern-greek-language.md)

### Post-v1 — Card order option (random / sequential)
- ✅ Added a per-session **card order** axis: `cardOrder: "random" | "sequential"`, default `random` (preserves prior behaviour). Sequential presents the selected Classic queue in deck order (`created_at` asc, tie-broken by card id) instead of shuffling; the challenge game types (boss/speed/review_blitz) keep their deliberate order and ignore the field. Threaded `validateSessionCreate` → `QueueOptions` → `buildQueue`; persisted as `card_order` on the session row (legacy rows default to `random` in `sessionProfile`). UI: Random / In order picker in `SessionSetup`; `StudySession` forwards `cardOrder` only when sequential (mirrors the `gameType !== "classic"` pattern)
  - Tests: 13 new (4 `card-priority.test.ts`, 3 `sessions-shared.test.ts`, 1 `sessions.test.ts`, 3 `SessionSetup.test.jsx`, 2 `StudySession.test.jsx`). 934 api + 645 frontend pass. Coverage `card-priority.ts` 100 / `sessions.ts` 95 / `sessions-shared.ts` 100 (Tier A); `SessionSetup.jsx` 96.8 / `StudySession.jsx` 93.8 / `strings.js` 100 (Tier B). No new deps. Security scan PASS (no auth/Claude/secret seam touched; invariant meta-tests green). Plan: [docs/plans/done/post-v1-card-order.md](docs/plans/done/post-v1-card-order.md)

### Post-v1 — Never "No cards due": hardest-first fallback queue
- ✅ Killed the dead-end where a fully-learned course returns an empty study queue ("No cards due. Come back later!"). `buildQueue` now wraps its per-mode selection with a guard: when the mode's normal selection is empty **but the course still has cards**, it falls back to the whole deck ordered **hardest-first by SM-2 ease** (ties broken by card id), with the never-failed "easy" cards (`ease ≥ 2.5`) shuffled in behind via the `random` seam — "most difficult first, stuffed by random easy ones". One new helper (`fallbackHardestFirst`) + one guard in `card-priority.ts`; numeric-`cardLimit` paths already backfilled and are untouched, and a genuinely empty course (0 cards, or MCQ with no distractor-capable cards) still shows the empty screen. Difficulty signal is `sm2_ease` only (card-only, no attempts read), per user choice. No API/UI/SM-2 changes
  - Tests: 6 new (`card-priority.test.ts` never-empty block incl. ordering, easy-tail shuffle, tie-break, 4× game-type non-empty, empty-deck guard) + 4 existing assertions flipped (3 `card-priority.test.ts`, 1 `sessions.test.ts`). 942 api + 645 frontend pass. Coverage `card-priority.ts` 100 lines / 95.77 branches (Tier A). No new deps. Security scan skipped per autonomous rule (pure ordering logic, no auth/Claude/secret seam). Plan: [docs/plans/done/post-v1-never-empty-hardest-first.md](docs/plans/done/post-v1-never-empty-hardest-first.md)

### Post-v1 — Hardest-first default order + always-full Classic deck
- ✅ Promoted hardest-first from the empty-only fallback to a **first-class, default card order**, and made Classic **always pad to a full deck**. `CardOrder` is now a 3-way axis: **`hardest_first`** (new default — hardest cards lead by `sm2_ease`, never-failed `ease ≥ 2.5` cards shuffled into the tail), **`random`** (pure shuffle, unchanged), **`sequential`** (deck order, unchanged). Classic's "All" path now returns the **entire deck** (was due + ≤20 new), and the numeric-limit path already padded — so every Classic session leads with what you struggle with, then stuffs random easy ones, never thin/empty. Reused the shipped `orderHardestFirst` (renamed from `fallbackHardestFirst`) for both the default order and the never-empty guard; dropped the now-dead `MAX_NEW_CARDS`. Boss/Speed/Review Blitz keep their deliberate orders. Frontend: 3-button picker `[Hardest first] [Random] [In order]` (Hardest first default), `StudySession` omits only the new default from the request body, 2 new i18n keys per locale. API default + validation message updated; legacy session rows still read back as `random` (historically accurate)
  - Tests: 5 new + 8 flipped across `card-priority.test.ts` (full-deck/null, hardest-first ordering, omitted-default), `sessions-shared.test.ts` (default `hardest_first`, accepts new value), `sessions.test.ts` ("All" includes learned cards / no 20-cap), `SessionSetup.test.jsx` (default selected, Random chosen), `StudySession.test.jsx` (random sent, default omitted). 944 api + 647 frontend pass. Coverage `card-priority.ts` 100 lines / 95.89 branches, `sessions-shared.ts` 100, `sessions.ts` 95 (Tier A); `SessionSetup.jsx` 96.8 / `StudySession.jsx` 93.8 / `strings.js` 100 (Tier B). `tsc` build clean. No new deps. Security scan skipped per autonomous rule (ordering/validation/UI only). Plan kept in-context (autonomous slice)

### Post-v1 — Sound off by default
- ✅ The TTS speaker toggle (`speechOn`) now defaults **off** in both StudySession and CardManager (was on), so 🔊 audio doesn't start on. In StudySession it initializes from the user's `auto_speak` setting — so anyone who explicitly opted into auto-speak keeps sound on (no silent regression), everyone else starts at 🔇. CardManager (no auto-speak) defaults plainly off. Two-line source change (`useState(true)` → `useState(() => Boolean(user?.settings?.auto_speak))` / `useState(false)`); the manual-speak-button tests now enable sound via the speaker toggle first
  - Tests: 4 new (default-off + auto-speak-on no-regression in `StudySession.test.jsx`; default-off in `CardManager.test.jsx`) + 11 flipped to toggle sound on first. 650 frontend pass. Coverage `StudySession.jsx` 93.8 / `CardManager.jsx` 95.4 lines (Tier B). No API change, no new deps. Security scan N/A (pure UI default, no auth/Claude/secret seam)
