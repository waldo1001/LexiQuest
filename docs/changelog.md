...
- Added Kaat (`#f59e0b`, 🐰) and Amaryllis (`#ec4899`, 🌸) to `SEED_USERS` and made `GET /api/users/public` filter `is_admin === true` rows so Waldo (kept as admin / supervisor) is hidden from the student picker. 6 new tests across `seed.test.ts` and `users-public.test.ts`; 100% coverage on both touched source files. `docs/setup.md` and `docs/deployment.md` (§1c, §1d, new §1e "Adding new family users to live") document the two new `PASSWORD_KAAT` / `PASSWORD_AMARYLLIS` env vars. See [plans/seed-users-kaat-amaryllis.md](plans/seed-users-kaat-amaryllis.md).
